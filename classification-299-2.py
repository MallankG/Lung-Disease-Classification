...
# Transformations for 299x299 input size
...
# Dataset and Dataloader for 299x299
batch_size = 32
...
print("✅ Data loaders created for input size 299.")

# Model Definitions - Models for 299x299 input
...
# Models expecting 299x299 input
models = {
...
print("299x299 ->", list(models.keys()))
...
def train_and_validate(model, model_name, combo_id, combo, dataloaders, device, class_names):
    ...
    results_dir = f"results-2/combo_{combo_id}"
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(dataloaders['train'], desc=f"Epoch {epoch+1} Training"):
            ...
            optimizer.zero_grad()
            # For InceptionV3, handle auxiliary outputs
        ...
        with torch.no_grad():
            for inputs, labels in tqdm(dataloaders['val'], desc=f"Epoch {epoch+1} Validation"):
                inputs, labels = inputs.to(device), labels.to(device)
                # Handle InceptionV3 for validation (no aux_logits needed)
...
def evaluate_and_visualize(model, model_name, combo_id, model_path, dataloaders, class_names, device):
    ...
    with torch.no_grad():
        for inputs, labels in dataloaders['test']:
            ...
            # Handle InceptionV3 for testing
            if model_name == "inceptionv3":
                outputs = model(inputs)
                if isinstance(outputs, tuple):
                    outputs = outputs[0]
...
if __name__ == "__main__":
    ...
    for model_name, model in models.items():
        for combo_id, combo in enumerate(combos, 1):
            print(f"🔥 Training {model_name} | Input 299 | Combo {combo_id}")
            model.to(device)
    ...
    # === EVALUATION LOOP ===
    for model_name, model in models.items():
        print(f"🧠 Evaluating {model_name} for input size 299")
        for combo_id, combo in enumerate(combos, 1):
            results_dir = f"results-2/combo_{combo_id}"
    ...
    metrics_df.to_csv("results-2/metrics_summary_299_d1.csv", index=False)
    print("✅ Training and Evaluation completed for all 299x299 input models.")
